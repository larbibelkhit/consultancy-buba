  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ background: 'radial-gradient(ellipse at 70% 40%, rgba(184,115,51,0.07) 0%, transparent 60%)' }}
      />
      <div
        className="absolute inset-0 pointer-events-none"
        style={{ background: 'radial-gradient(ellipse at 20% 80%, rgba(184,115,51,0.04) 0%, transparent 50%)' }}
      />

      <p className="text-[0.65rem] tracking-[0.35em] uppercase text-gold mb-5">
      ...
      <h1 className="font-heading font-light text-[3.5rem] md:text-[5rem] lg:text-[7rem] leading-[1.05] tracking-[-0.01em] text-ivory max-w-[800px]">
        ...
        <br />
        that <em className="italic text-gold">endure.</em>
      </h1>
      ...
      <a
        href="#contact"
        className="mt-12 inline-flex items-center gap-4 text-[0.7rem] tracking-[0.25em] uppercase text-gold group hover:gap-6 transition-[gap] duration-300"
      >
        Begin a conversation
        <span className="block w-10 h-px bg-gold group-hover:w-14 transition-[width] duration-300" />
      </a>
